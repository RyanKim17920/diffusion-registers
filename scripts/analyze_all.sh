...
for d in "$RUNS"/phase1_k*_s*; do
  ...
  k=$("${REG_PYTHON:-python}" -c "import json,sys;print(json.load(open(sys.argv[1]))['n_registers'])" "$d/model_config.json" 2>/dev/null) || continue
  [ -f "$d/analysis_val.json" ] && { echo "skip $(basename $d) (already analysed)"; continue; }
  echo "=== analysing $(basename "$d") (K=$k) ==="
  "${REG_PYTHON:-python}" "$REPO/src/analyze.py" --run "$d" --n_solve "$N_SOLVE" --n_attn 256 || echo "ANALYSIS_FAILED $d"
done
